...
const Filter = () => {
  ...
  const searchCcollection = async (data) => {
    ...
    try {
      ...
      } = await axiosClient.post('/collection/filter', data);
      setCollect(content);
    } catch (error) {
      console.log(error);
    }
      ...
      <div className="container">
        <Box className="flex justify-center items-center gap-5">
          ...
          <Box className="flex justify-center">
            {' '}
            <Button
              onClick={handleSubmit(searchCcollection)}
              className="text-white mt-1 bg-gradient-to-r from-cyan-400 via-cyan-500 to-cyan-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-cyan-300  font-medium rounded-lg text-sm px-10 py-3 text-center mr-2 mb-2"
            >
              Search
            </Button>
          </Box>
